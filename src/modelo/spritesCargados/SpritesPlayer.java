...
public class SpritesPlayer extends Animation {
	...
	private void loadGifs() {
		...
		for (int i = 0; i < Assets.gifs.size(); i++) {
			Gif gif = new Gif(Assets.spriteNames[i], Assets.gifs.getImages(Assets.spriteNames[i]),
		...
		for (int i = 0; i < Assets.gifsL.size(); i++) {
			Gif gif = new Gif(Assets.legNames[i], Assets.gifsL.getImages(Assets.legNames[i]), this.getTransformar(),
					150);
