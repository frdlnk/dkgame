...
public class EnemigoPistola extends Enemigo {
	...
	/**
	 * Crea un nuevo enemigo con {@value #VIDA} de vida y una pistola con
	 * {@value #MUNICIONES} balas
	 * 
		...
		if (isInScreen()) {
			...
			if (huir) {
				...
				if (distanciaJugador != null) {
					// huye hasta verificar que se va a salir del mapa o esta los suficientemente
					// lejos de l jugador
	...
	private boolean puedeHuir(Vector2D distanciaJugador) {
		...
		return Math.abs(distanciaJugador.getX()) < 120 && transformar.getPosicion().getX() > 30
